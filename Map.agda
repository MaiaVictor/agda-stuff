open import Prelude.List hiding (lookup; insert)
...
insert : {A : Set} -> {n : Nat} -> Bits n -> A -> Map A -> Map A
...
lookup : {A : Set} -> {n : Nat} -> Bits n -> Map A -> Maybe A
lookup []       leaf         = nothing
lookup []       (node a x y) = a
...
delete : {A : Set} -> {n : Nat} -> Bits n -> Map A -> Map A
delete []      leaf         = leaf
